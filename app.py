            ...
            while completed_images_count < count :
                ...
                if 'gemini' in  template_str.lower()  : # gemini
                    ...
                    if template_str == 'gemini_redraw':
                        ...
                        if is_pd_policy:
                            gemini_prompt = gemini_text_lib(f"given (keep original text overlay text language!!) this image prompt, keep it as close to input as possible!!! dont change the visuals, but follow the following rules, if some element dosent comply, remove it, input{gemini_prompt} rules, return JUST the output no intros:" + predict_policy,model = "gemini-2.5-flash-preview-04-17")
