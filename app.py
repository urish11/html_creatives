...
def upload_pil_image_to_s3(
    ...
    try:
        ...
        image.save(img_byte_arr, format=image_format, quality= 90)
        img_byte_arr.seek(0)
...
def gemini_text_lib(prompt,model ='gemini-2.5-pro-preview-03-25', is_with_file=False,file_url = None ): # gemini-2.5-pro-exp-03-25
    if is_pd_policy_global : prompt += PREDICT_POLICY
    st.text(prompt)
#st.text(prompt)
               ...
               if "google" in template.lower() or "google" in topic_for_api.lower(): # Simplified check
                   ...
                   if template_str == 'gemini2':
                       ...
                       gemini_api_prompt = grok_text(f"""write short prompt for\ngenerate square image promoting '{topic}' in language {lang} . add a CTA button with
                                                                'Learn More Here >>' in appropriate language\\n \n always use high CTR  overlay text on image (also the CTA text ), \nshould be low quality and very enticing and alerting design!! \nstart with 'square image aspect ratio of 1:1 of '\n\n be specific in what is shown . return JUST the best option, no intros
                                                                """)
                       ...
                       gemini_api_prompt = claude(f"""write short prompt for\ngenerate square image promoting '{topic}' in language {lang} . add a CTA button with
                                                        'Learn More Here >>' in appropriate language\ \n use  a curiosity-driven 1 sentnece   that emphasizes  interest in solutions related to the topic. Keep the tone persuasive and newsy NOT CLICKBAITY and as a question if approriate convincing . make it highy engaging and curisty evoking like (example) 'Why More People Are Looking To These Options For .. '\n \nshould be low quality design and very enticing and alerting!!, don't make specific promises like x% discount and 'act fast' or 'limited available'  \n\nstart with 'square image aspect ratio of 1:1 of '\n\n be specific in what is shown . return JUST the best option, no intros
                                                        if you want to add a caption, specifically instruct it to be on the image. and be short
                                                        """, is_thinking=False)
                       ...
                       gemini_api_prompt = claude(f"""write short prompt for\ngenerate square image promoting '{topic}' in language {lang}  . add a CTA button with
                                                        'Learn More Here >>' in appropriate language  \ \nshould be low quality and very enticing and alerting design!!!! BUT simple  layout , make the text legable and in negative space, DO NOT make specific promises like % discount and NEVER use overlay text in this style: 'in fraction of the price' or 'act fast' or 'limited available' 'weekly pay' 'Start today' 'Start Earning This Week' 'Flexible Hours' 'Stable Work'.  dont write promises you are not sure about!! !     \n\nstart with 'square image aspect ratio of 1:1 of '\n\n be specific in what is shown . return JUST the best option, no intros
                                                        if you want to add a caption, specifically instruct it to be on the image. and be short
                                                        """, is_thinking=False)
...
if st.session_state.get('ad_creation_processing_active') and st.session_state.ad_creation_task_queue:
    ...
    try:
        ...
        final_ad_s3_url = upload_pil_image_to_s3(
            ...
            region_name=AWS_REGION_SECRET, image_format='JPG' # Screenshots are PNG
        )
