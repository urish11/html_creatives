...
df = st.data_editor(
    pd.DataFrame({"topic": ["example_topic"], "count": [1], "lang": ["english"], "template": ["2,3,4,41,42,5,6,7,gemini,gemini2,gemini7,gemini7claude,geminicandid | use , for random template"]}),
    num_rows="dynamic",
    key="table_input"
)
            ...
            while completed_images_count < count :
                ...
                if 'gemini' in  template_str.lower()  : # gemini
                    ...
                    if template_str == 'geminicandid':
                        gemini_prompt = claude(f"""write a image prompt of a candid unstaged photo taken of a regular joe showing off his\her {topic} ('if showing off' it isnt appropriate, think of a better way). the image is taken with smartphone candidly. in 1-2 sentences. Describe the quality of the image looking smartphone. start with "Square photo 1:1 iphone 12 photo uploaded to reddit:"
