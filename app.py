        ...
        for entry in st.session_state.generated_images:
            ...
            for idx, img in enumerate(selected_images):  # Generate HTML with the selected image
                for i in range(img['selected_count']):  # Generate multiple times based on user input
                    ...
                    if template == 5 :
                        tag_line = chatGPT(f'write a tag line for {re.sub('\|.*','',topic)} in language {lang}, short and consice, to drive action. For example "⛔ Never Ignore These ⛔" with appropriate emojis, not necessarily the No Entry emoji!! only if it fits  \ndont mention the topic explicitly, rather drive action').strip('"').strip("'").strip("!")
                    else : tag_line = ''
