            ...
            while completed_images_count < count :
                ...
                if 'gemini' in  template_str.lower()  : # gemini
                    ...
                    if template_str == 'gemini7batch': # gemini1 with geimini text
                        gemini_prompt = gemini_text_lib(f"""Generate {count} distinct image generation prompts based on the following criteria. The final output MUST be ONLY a JSON object where keys are sequential integers starting from 0, and values are the complete prompt strings. Do not include any introductory text or explanations.\n\n**Core Task:** Create image prompts promoting '{topic}' in the language '{lang}'.\n\n**Prompt Requirements (for each generated prompt):**\n1.  **Starting Phrase:** Each prompt MUST begin *exactly* with: `square image aspect ratio of 1:1 of `\n2.  **Visual Style:** Describe an image that is visually **highly enticing and alerting**, using a **deliberate low-quality aesthetic**. Think slightly pixelated, perhaps overly saturated or garish colors, maybe reminiscent of urgent flyers, old low-res internet ads, or slightly distorted graphics. Be specific in describing *how* this aesthetic is achieved (e.g., 'noticeable pixelation', 'vibrant clashing colors', 'slightly blurry motion effect').\n3.  **Content Specificity:** Provide a detailed and specific description of the visual elements within the scene related to '{topic}'. What is happening? What objects or figures are present? What is the background? What mood is conveyed through the visuals?\n4.  **Call to Action (CTA):** Explicitly describe the inclusion of a prominent CTA button within the image. Specify its appearance (e.g., 'a bright yellow rectangular button', 'a flashing red circle button') and placement. The text on the button MUST be the '{lang}' translation of 'Learn More Here >>'.\n5.  **Detail and Length:** Each prompt should be rich in detail to guide the image generation effectively, aiming for descriptive depth (implicitly aiming for the length you previously mentioned, around 500 characters, by demanding detail, but without a hard character count constraint).\n6.  **Creativity and Variance:** Ensure significant variation between the {count} prompts. They should explore different visual concepts, scenarios, compositions, or metaphors related to the '{topic}' while adhering to the requested style.\n\n**Output Format:** Return ONLY the JSON object containing the prompts. as json key is int index , it's value is the prompt"""
